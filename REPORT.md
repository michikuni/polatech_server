...
Máy chủ **tự dò địa chỉ IP của chính nó** và in ra mã QR trên trang quản trị; cán bộ chỉ việc quét. Kể cả khi máy chủ khởi động lại và đổi sang IP khác, cán bộ chỉ cần quét lại QR — không cần cấp mã ghép cặp mới, không mất khoá bảo mật đã có. Nếu không quét được QR có thể nhập IP thủ công
...
| Thành phần | Tên công nghệ | Giải thích |
|---|---|---|
...
|---|---|---|
| Ngôn ngữ | **Kotlin** (chạy trên nền Java 17) | Ngôn ngữ lập trình hiện đại, an toàn. Chạy trên nền Java — nền tảng đã được kiểm chứng hàng chục năm trong ngân hàng, viễn thông, cơ quan nhà nước. |
...
### Hạ tầng (chi phí phần mềm: **0 đồng** — toàn bộ công nghệ đều miễn phí)
